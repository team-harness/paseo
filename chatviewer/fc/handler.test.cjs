...
const environment = {
  CHAT_SHARE_OSS_ACCESS_KEY_ID: "test-key",
  CHAT_SHARE_OSS_ACCESS_KEY_SECRET: "test-secret",
  CHAT_SHARE_OSS_BUCKET: "test-bucket",
...
function createTestHandler() {
  ...
  return {
    handler: createHandler({
      ...
      assets: {
        "/index.html": {
          body: Buffer.from("<main>Paseo Chat</main>").toString("base64"),
...
test("serves the viewer from the same FC origin", async () => {
  ...
  assert.equal(response.body.toString("utf8"), "<main>Paseo Chat</main>");
});
