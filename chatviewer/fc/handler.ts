...
function ossRequest(
  ...
  const bucket = environmentValue(environment, "CHAT_SHARE_OSS_BUCKET");
  const region = environmentValue(environment, "CHAT_SHARE_OSS_REGION");
  const accessKeyId = environmentValue(environment, "CHAT_SHARE_OSS_ACCESS_KEY_ID");
  const accessKeySecret = environmentValue(environment, "CHAT_SHARE_OSS_ACCESS_KEY_SECRET");
