...
describe("AgentStorage", () => {
  ...
  describe("per-turn outcomes", () => {
    ...
    test("persists the accepted turn identity atomically with the active marker", async () => {
      await storage.applySnapshot(createManagedAgent({ id: "agent-1", cwd: "/tmp/project" }));
      ...
      await storage.setActiveTurn(
        ...
        { clientMessageId: "team-mission:mission-1:assignment-1:dispatch" },
      );

      expect(
        await storage.getAcceptedTurnId("agent-1", "team-mission:mission-1:assignment-1:dispatch"),
      ).toBe("turn-accepted-1");
      expect((await storage.get("agent-1"))?.activeTurn?.turnId).toBe("turn-accepted-1");

      await storage.applySnapshot(createManagedAgent({ id: "agent-1", cwd: "/tmp/project" }));

      const reloaded = new AgentStorage(storagePath, logger, { runId: storage.runId });
      await reloaded.initialize();
      expect(
        await reloaded.getAcceptedTurnId("agent-1", "team-mission:mission-1:assignment-1:dispatch"),
      ).toBe("turn-accepted-1");
    });

    test("persists an unknown acceptance fence before entering the provider", async () => {
      await storage.applySnapshot(createManagedAgent({ id: "agent-1", cwd: "/tmp/project" }));
      const clientMessageId = "team-mission:mission-1:assignment-1:dispatch";

      expect(await storage.claimPromptDispatch("agent-1", clientMessageId)).toMatchObject({
        kind: "claimed",
      ...
      const reloaded = new AgentStorage(storagePath, logger);
      await reloaded.initialize();
      expect(await reloaded.claimPromptDispatch("agent-1", clientMessageId)).toEqual({
        kind: "unknown",
      });

      await reloaded.setActiveTurn(
        "agent-1",
        {
          turnId: "turn-accepted-1",
          startedAt: "2026-08-06T10:00:00.000Z",
    ...
    test("releases only the prompt dispatch claim owned by the matching token", async () => {
      await storage.applySnapshot(createManagedAgent({ id: "agent-1", cwd: "/tmp/project" }));
      const clientMessageId = "team-mission:mission-1:assignment-1:dispatch";

      const first = await storage.claimPromptDispatch("agent-1", clientMessageId);
      expect(first).toMatchObject({ kind: "claimed", token: expect.any(String) });
      if (!first || first.kind !== "claimed") throw new Error("Expected a dispatch claim");

      await expect(
        storage.releasePromptDispatchClaim("agent-1", clientMessageId, "another-owner-token"),
      ).resolves.toBe(false);
      expect(await storage.claimPromptDispatch("agent-1", clientMessageId)).toEqual({
        kind: "unknown",
      });

      await expect(
        storage.releasePromptDispatchClaim("agent-1", clientMessageId, first.token),
      ).resolves.toBe(true);
      const retried = await new AgentStorage(storagePath, logger).claimPromptDispatch(
        "agent-1",
        clientMessageId,
      );
      expect(retried).toMatchObject({ kind: "claimed", token: expect.any(String) });
