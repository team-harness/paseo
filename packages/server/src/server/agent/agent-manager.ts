...
interface TurnSettlementIntent {
  turnId: string;
  outcome: TurnOutcome["outcome"];
...
export class AgentManager {
  ...
  private readonly closingSessionIngress = new Set<string>();
  ...
  private readonly turnSettlementIntents = new Map<string, TurnSettlementIntent>();
  ...
  private async closeAgentRuntime(agentId: string): Promise<void> {
    ...
    this.closingSessionIngress.add(agentId);
    ...
    try {
      ...
      await this.whenTurnStateSettled(agentId);
      await this.retryTurnSettlement(agentId);
      if (this.registry && agent.activeTurnId) {
        this.applyActiveTurnTerminal(agent, undefined, false, "canceled");
        await this.whenTurnStateSettled(agentId);
      }
      this.cancelRunningProviderSubagents(agentId);
    } catch (error) {
      this.closingSessionIngress.delete(agentId);
  ...
  streamAgent(
    ...
    const streamForwarder = async function* streamForwarder(this: AgentManager) {
      ...
      const acceptedTurnWrite = this.openActiveTurn(
        agent,
        turnId,
        turnStartedAt,
      ...
      try {
        await this.waitForDurableAcceptedTurn(options?.clientMessageId, acceptedTurnWrite);
  ...
  private async waitForDurableAcceptedTurn(
   ...
   *
   * The write is queued after the foreground waiter exists, then awaited before
   * the stream yields `turn_started`. The active marker and client-message
   * mapping therefore form the durable acceptance boundary without losing a
   * terminal event from a provider that finishes immediately.
   */
  private openActiveTurn(
    ...
    const registry = this.registry;
    const write = (async () => {
      const persisted = await registry.setActiveTurn(
        agent.id,
        {
          turnId,
  ...
  async getAcceptedTurnId(agentId: string, clientMessageId: string): Promise<string | null> {
    await this.whenTurnStateSettled(agentId);
    return (await this.registry?.getAcceptedTurnId(agentId, clientMessageId)) ?? null;
  }
  ...
  private settleTurnRecord(
    ...
    if (!turnId) return;
    const intent: TurnSettlementIntent = { turnId, outcome, attempt: null };
    this.turnSettlementIntents.set(agentId, intent);
    this.persistTurnSettlement(agentId, intent);
  }

  private persistTurnSettlement(agentId: string, intent: TurnSettlementIntent): Promise<void> {
    if (intent.attempt) return intent.attempt;
    const { turnId, outcome } = intent;
    const settle = Promise.resolve().then(async () => {
      ...
      if (this.turnSettlementIntents.get(agentId) === intent) {
        this.turnSettlementIntents.delete(agentId);
      }
      return undefined;
    });
    intent.attempt = settle;
    this.trackTurnStateWrite(agentId, settle);
    const clearAttempt = () => {
      if (intent.attempt === settle) {
        intent.attempt = null;
      }
    };
    void settle.then(clearAttempt, clearAttempt);
    return settle;
  }

  private async retryTurnSettlement(agentId: string): Promise<void> {
    const intent = this.turnSettlementIntents.get(agentId);
    if (!intent) return;
    await this.persistTurnSettlement(agentId, intent);
  }
  ...
  private enqueueSessionEvent(agentId: string, event: AgentStreamEvent): void {
    if (this.closingSessionIngress.has(agentId)) {
      return;
    }
    this.logger.trace(
      {
  ...
  private dispatchStreamEventByType(params: {
    ...
    switch (event.type) {
      ...
      case "model_changed":
        flags.shouldDispatchEvent = false;
        this.onStreamModelChanged(agent, event);
        return undefined;
  ...
  private onStreamTurnCompleted(params: {
    ...
    const { agent, event, eventTurnId, isForegroundEvent, fromHistory, terminalDisposition } =
      params;
    this.logger.trace(
      {
        agentId: agent.id,
