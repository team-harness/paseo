...
async function rebindCommandCenter(page: Page, combo: string): Promise<void> {
  await page.addInitScript(
    ([key, bindingIds, value]) => {
      // Exercise the same custom chord on both macOS and Linux CI.
      window.localStorage.setItem(
        key,
        JSON.stringify(Object.fromEntries(bindingIds.map((id) => [id, value]))),
      );
    },
    [OVERRIDE_STORAGE_KEY, COMMAND_CENTER_BINDINGS, combo] as const,
  );
