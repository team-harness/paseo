...
function ComposerContentImpl({
  ...
  const textSelectionRef = useRef<TextSelection>({
    start: textSource.getSnapshot().length,
    end: textSource.getSnapshot().length,
  });
  ...
  useEffect(
    () =>
      textSource.subscribe(() => {
        const value = textSource.getSnapshot();
        ...
        if (historyAppliedValue !== value) messageHistoryIndexRef.current = null;
      }),
    [textSource],
  );
  ...
  const handleCommandKeyPress = useCallback(
    (event: ComposerKeyPressEvent) => {
      ...
      cursor.setState(navigation.selection.start);
    ...
    [messageHistory, replaceUserInput, textSource, cursor],
  );
  ...
  const handleInsertText = useCallback(
    (content: string, kind: "text" | "quote" = "text") => {
      const userInput = textSource.getSnapshot();
      ...
      cursor.setState(result.selection.start);
    ...
    [replaceUserInput, textSource, cursor],
  );
