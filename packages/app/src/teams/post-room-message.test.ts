...
function gateway(answer: { message: ChatMessage | null; error: string | null } | Error) {
  return {
...
describe("saying something in a team's room", () => {
  it("goes through pending and back to idle", async () => {
    ...
    await postRoomMessage(
      { roomId: "room-1", body: "hello", client: gateway({ message: message(), error: null }) },
      LABELS,
  ...
  it("sends the room and the body it was given", async () => {
    ...
    await postRoomMessage({ roomId: "room-1", body: "hello", client }, LABELS, () => {});

    expect(client.postChatMessage).toHaveBeenCalledWith({ room: "room-1", body: "hello" });
  });
  ...
  it("says something when the daemon refuses without saying why", async () => {
    ...
    await postRoomMessage(
      { roomId: "room-1", body: "hello", client: gateway({ message: null, error: null }) },
      LABELS,
      (state) => seen.push(state),
    );
  ...
  it("reports a request that never got an answer", async () => {
    ...
    await postRoomMessage(
      { roomId: "room-1", body: "hello", client: gateway(new Error("The connection dropped")) },
      LABELS,
      (state) => seen.push(state),
    );
  ...
  it("refuses to send with no host rather than doing nothing", async () => {
    ...
    await postRoomMessage({ roomId: "room-1", body: "hello", client: null }, LABELS, (state) =>
      seen.push(state),
    );
  ...
  it("does not send an empty message", async () => {
    ...
    await postRoomMessage({ roomId: "room-1", body: "   ", client }, LABELS, (state) =>
      seen.push(state),
    );

    expect(client.postChatMessage).not.toHaveBeenCalled();
    expect(seen).toEqual([]);
  });
